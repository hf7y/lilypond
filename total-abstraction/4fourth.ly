...
FOURTHsiiA = \relative c'' {
  ...
  \tempo \markup \small {" as if reciting Shakespeare"}
  \bar ".|:" \no-line {s2 | s1 | s2 } \bar ":|."
  \fat-line { s2 | s1 }
}
...
FOURTHsiB = \relative c'' {
  \FOURTHtuttiB
  \stemUp
  \fat-line { s1 | s1 | \stemless \clicks f4:8 s2. }
  \force-clef-treble \stemless \parenthesize a,4 
    \fat-line { \stemless \clicks f'4:16 s2 | }
    \fat-line { s1  | s1 }
  \force-clef-treble \stemless \parenthesize cis4
    \fat-line { s2. | s1 | \stemless \clicks f4:32  | s2. }
}
FOURTHsiBLyrics = \lyricmode {
  \skip 1 \skip 1 \skip 1
  \markup { \box "ɑ" "→"}1 \skip 1  \skip 1
  \markup { \box "ʌ" "→" }1 \skip 1 \markup \box "z"1
}
...
FOURTHsiiB = \relative c'' {
  ...
  \fat-line { s2 \stemless \clicks f4:16 s4 | s1  | s1 }
  \fat-line { s1 | s2. \stemless \clicks f4:32 | s1 }
}
FOURTHsiiBLyrics = \lyricmode {
  \skip 1 \skip 1 \skip 1
  \markup \small "becoming discrete words→"1 \skip 1 \skip 1 
  \skip 1 \skip 1 \skip 1
}
...
FOURTHaiB = \relative c' {
  \FOURTHtuttiB
  \stemUp
  \fat-line { s1 | s2 \stemless \clicks f'4:8 s4 | s1 }
  \fat-line { s2. \stemless \clicks f4:16 | s1  | s1 }
  \force-clef-treble \stemless \parenthesize d,4
    \fat-line { s2. | s2 \stemless \clicks f'4:32 s4 | s1 }
}
...
FOURTHaiiB = \relative c'' {
  \FOURTHtuttiB
  \stemUp
  \fat-line { s1 | s4 \stemless \clicks f4:8 s2 | s1 }
  \fat-line { s1 | \stemless \clicks f4:16  s2. | s1 }
  \fat-line { s1 | s4 \stemless \clicks f4:32 s2 | s1 }
}
...
FOURTHtiB = \relative c' {
  ...
  \fat-line { s1 | \stemless \clicks f4:32 s2. | s1 }
}
...
FOURTHtiiB = \relative c' {
  ...
  \force-clef-tenor \stemless \parenthesize g4
    \fat-line { s2 \stemUp \stemless \clicks f4:8 | }
    \fat-line { s1 | s1 }
  \force-clef-tenor \stemless \parenthesize d4
    \fat-line { s2. | s2 \stemless \clicks f4:16  s4  | s1 }
  \fat-line { s2. \stemUp \stemless \clicks f4:32 | }
    \fat-line { s1 | s1 }
}
...
FOURTHbiB = \relative c' {
  \FOURTHtuttiB
  \stemUp
  \fat-line { s2 \stemless \clicks a4:8 s4 | s1 | s1 }
  \fat-line { s1 | s2. \stemless \clicks f4:16  | s1 }
  \fat-line { s2 \stemless \clicks a4:32 s4 | s1 | s1 }
}
...
FOURTHbiiB = \relative c {
  ...
  \force-clef-bass \stemless \parenthesize cis,4
    \fat-line { s2. | s1 | \stemless \clicks f4:16 s2. }
